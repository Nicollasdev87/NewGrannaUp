...
const Transactions: React.FC<TransactionsProps> = ({
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mt-6">
        ...
        <div className="flex flex-wrap gap-3">
          ...
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2.5 rounded-lg border border-slate-200 dark:border-slate-700 ${showFilters ? 'bg-primary/10 border-primary/30 text-primary' : 'bg-white dark:bg-slate-800'} text-sm font-medium hover:bg-slate-50 transition-all`}
          >
      ...
      {showFilters && (
        <div className="p-4 bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 grid grid-cols-1 md:grid-cols-3 gap-4 animate-in slide-in-from-top-4 duration-300">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="bg-white dark:bg-slate-800 p-5 rounded-xl border border-slate-200 dark:border-slate-700 flex items-center justify-between">
          ...
          <span className="px-2 py-1 bg-green-50 text-green-600 text-xs font-bold rounded-full">Recebido</span>
        </div>
                ...
                <tr key={tx.id} className={`group hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors ${selectedIds.has(tx.id) ? 'bg-primary/5' : ''}`}>
                  ...
                  <td className="p-4">
                    <div className="flex items-center gap-3">
                      <div className={`w-9 h-9 rounded-full flex items-center justify-center text-lg ${tx.type === 'income' ? 'bg-green-50 text-green-600' :
                  ...
                  <td className="p-4 text-center">
                    <span className={`px-2 py-1 text-[10px] font-bold rounded-full uppercase ${tx.type === 'income' ? 'bg-green-50 text-green-700' :
                      tx.type === 'investment' ? 'bg-purple-50 text-purple-700' :
                  ...
                  <td className={`p-4 text-right text-sm font-bold ${tx.type === 'income' ? 'text-green-600' :
